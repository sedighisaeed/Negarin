...
<body class="bg-white">
    <div class="embed-card">
        <div class="card status-card-embed card-md-rounded-0 border">
            <div class="card-header d-inline-flex align-items-center justify-content-between bg-white">
                ...
                <div>
                    <a class="small font-weight-bold text-muted pr-1" href="{{config('app.url')}}" target="_blank">{{config('negarin.domain.app')}}</a>
                    <img src="/img/negarin-icon-color.svg" width="26" height="26">
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        fetch("{{config('app.url')}}/api/negarin/v1/accounts/{{$profile['id']}}/statuses?only_media=true&limit=24")
        .then(res => res.json())
        .then(res => {
