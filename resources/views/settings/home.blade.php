...
@section('section')
	...
	<form method="post">
		...
		<div class="form-group row">
			...
			<div class="col-sm-9">
				<textarea
					...
					rows="2"
					data-max-length="{{config('negarin.max_bio_length')}}"
					maxlength="{{config('negarin.max_bio_length')}}"
					v-pre>{{strip_tags(Auth::user()->profile->bio)}}</textarea>
				<p class="form-text">
					<span class="bio-counter float-right small text-muted">0/{{config('negarin.max_bio_length')}}</span>
				</p>
			</div>
		</div>
		<div class="form-group row">
        ...
        @endif
        @endif
		@if(config_cache('negarin.enforce_account_limit'))
		<div class="pt-3">
...
$(document).ready(function() {
		...
		$('#maxAvatarSize').text(filesize({{config('negarin.max_avatar_size') * 1024}}, {round: 0}));
