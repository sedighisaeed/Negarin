...
class NotificationAppGatewayService
{
    ...
    {
        $endpoint = 'https://'.config('instance.notifications.nag.endpoint').'/api/v1/instance-check?domain='.config('negarin.domain.app');
        try {
            $res = Http::withHeaders(['X-negarin-API' => 1])
                ->retry(3, 500)
                ->throw()
                ->get($endpoint);
