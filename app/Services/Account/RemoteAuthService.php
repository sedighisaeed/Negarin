...
class RemoteAuthService
{
    ...
    public static function getMastodonClient($domain)
    {
        ...
        try {
            ...
            $res = Http::asForm()->throw()->timeout(10)->post($url, [
                'client_name' => config('negarin.domain.app', 'negarin'),
                'redirect_uris' => url('/auth/mastodon/callback'),
                'scopes' => 'read',
                'website' => 'https://negarin.org'
            ]);
    ...
    public static function isDomainCompatible($domain = false)
    {
        ...
        return Cache::remember(self::CACHE_KEY . 'domain-compatible:' . $domain, 14400, function() use($domain) {
            try {
                $res = Http::timeout(20)->retry(3, 750)->get('https://beagle.negarin.net/api/v1/raa/domain?domain=' . $domain);
                if(!$res->ok()) {
                    return false;
                }
            } catch (RequestException $e) {
    ...
    {
        try {
            $res = Http::timeout(20)->retry(3, 750)->get('https://beagle.negarin.net/api/v1/raa/lookup?webfinger=' . $wf);
            if(!$res->ok()) {
                return false;
            }
        } catch (RequestException $e) {
    ...
    {
        try {
            $url = 'https://beagle.negarin.net/api/v1/raa/submit';
