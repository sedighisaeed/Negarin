...
class ApiV1Controller extends Controller
{
    ...
    {
        abort_if(! (bool) config_cache('negarin.oauth_enabled'), 404);
    ...
    public function accountUpdateCredentials(Request $request)
    {
        ...
        if (config('negarin.bouncer.cloud_ips.ban_api')) {
            abort_if(BouncerService::checkIp($request->ip()), 404);
        }

        $this->validate($request, [
            'avatar' => 'sometimes|mimetypes:image/jpeg,image/jpg,image/png|max:'.config('negarin.max_avatar_size'),
            ...
            'avatar.max' => 'The :attribute exceeds the file size limit of '.PrettyNumber::size(config('negarin.max_avatar_size'), true, false),
        ]);
    ...
    public function instance(Request $request)
    {
        $res = Cache::remember('api:v1:instance-data-response-v1', 1800, function () {
            ...
            return [
                'uri' => config('negarin.domain.app'),
                'title' => config_cache('app.name'),
                'short_description' => config_cache('app.short_description'),
                'description' => config_cache('app.description'),
                'email' => config('instance.email'),
                'version' => '3.5.3 (compatible; negarin '.config('negarin.version').')',
                'urls' => [
                    'streaming_api' => null,
                ],
                'stats' => $stats,
                ...
                'registrations' => (bool) config_cache('negarin.open_registration'),
                'approval_required' => (bool) config_cache('instance.curated_registration.enabled'),
                'contact_account' => $contact,
                'rules' => $rules,
                'mobile_registration' => (bool) config_cache('negarin.open_registration') && config('auth.in_app_registration'),
                'configuration' => [
                    'media_attachments' => [
                        'image_matrix_limit' => 2073600,
                        'image_size_limit' => config_cache('negarin.max_photo_size') * 1024,
                        'supported_mime_types' => explode(',', config_cache('negarin.media_types')),
                        'video_frame_rate_limit' => 120,
                        'video_matrix_limit' => 2073600,
                        'video_size_limit' => config_cache('negarin.max_photo_size') * 1024,
                    ],
                    'polls' => [
                    ...
                    'statuses' => [
                        ...
                        'max_characters' => (int) config_cache('negarin.max_caption_length'),
                        'max_media_attachments' => (int) config_cache('negarin.max_album_length'),
                    ],
                ],
            ];
        });
    ...
    public function mediaUpload(Request $request)
    {
        ...
        $this->validate($request, [
            'file.*' => [
                ...
                'mimetypes:'.config_cache('negarin.media_types'),
                'max:'.config_cache('negarin.max_photo_size'),
            ],
            'file' => [
                'required_without:file.*',
                'mimetypes:'.config_cache('negarin.media_types'),
            ...
            'description' => 'nullable|string|max:'.config_cache('negarin.max_altext_length'),
        ]);
        ...
        if ((bool) config_cache('negarin.enforce_account_limit') == true) {
            $limit = (int) config_cache('negarin.max_account_size');
        ...
        $mimes = explode(',', config_cache('negarin.media_types'));
    ...
    public function statusCreate(Request $request)
    {
        ...
        $this->validate($request, [
            'status' => 'nullable|string|max:'.(int) config_cache('negarin.max_caption_length'),
            'in_reply_to_id' => 'nullable',
            'media_ids' => 'sometimes|array|max:'.(int) config_cache('negarin.max_album_length'),
            'sensitive' => 'nullable',
        ...
        if ($ids) {
            ...
            foreach ($ids as $k => $v) {
                if ($k + 1 > (int) config_cache('negarin.max_album_length')) {
                    continue;
                }
