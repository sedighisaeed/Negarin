...
class ApiV1Dot1Controller extends Controller
{
    ...
    public function report(Request $request)
    {
        ...
        if (config('negarin.bouncer.cloud_ips.ban_signups')) {
    ...
    public function accountChangePassword(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'new_password' => 'required|min:'.config('negarin.min_password_length', 8),
    ...
    /**
     * GET /api/v1.1/accounts/emails-from-negarin
     *
     * @return array
     */
    public function accountEmailsFromnegarin(Request $request)
    {
        abort_if(! $request->user() || ! $request->user()->token(), 403);
        abort_unless($request->user()->tokenCan('read'), 403);
    ...
    {
        return [
            'open' => (bool) config_cache('negarin.open_registration'),
            'iara' => (bool) config_cache('negarin.allow_app_registration'),
        ];
    }
    ...
    public function inAppRegistration(Request $request)
    {
        ...
        abort_unless((bool) config_cache('negarin.open_registration'), 404);
        abort_unless((bool) config_cache('negarin.allow_app_registration'), 404);
        abort_unless($request->hasHeader('X-negarin-APP'), 403);
        if (config('negarin.bouncer.cloud_ips.ban_signups')) {
            abort_if(BouncerService::checkIp($request->ip()), 404);
        }

        $rl = RateLimiter::attempt('pf:apiv1.1:iar:'.$request->ip(), config('negarin.app_registration_rate_limit_attempts', 3), function () {}, config('negarin.app_registration_rate_limit_decay', 1800));
    ...
    public function inAppRegistrationEmailRedirect(Request $request)
    {
        ...
        $url = 'negarin://confirm-account/'.$ut.'?'.$params;
    ...
    public function inAppRegistrationConfirm(Request $request)
    {
        ...
        $token = $user->createToken('negarin', ['read', 'write', 'follow', 'push']);
    ...
    public function accountUsernameToId(Request $request, $username)
    {
        ...
        if (str_ends_with($username, config_cache('negarin.domain.app'))) {
    ...
    {
        abort_unless($request->hasHeader('X-negarin-APP'), 404, 'Not found');
        abort_if(! $request->user() || ! $request->user()->token(), 403);
    ...
    {
        abort_unless((bool) config_cache('negarin.oauth_enabled'), 404);
