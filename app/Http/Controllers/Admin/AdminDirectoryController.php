...
trait AdminDirectoryController
{
    ...
    public function directoryInitialData(Request $request)
    {
        ...
        $config = ConfigCache::whereK('negarin.directory')->first();
        if ($config) {
        ...
        $res['open_registration'] = (bool) config_cache('negarin.open_registration');
        $res['oauth_enabled'] = (bool) config_cache('negarin.oauth_enabled') &&
        ...
        $res['feature_config'] = [
            'media_types' => Str::of(config_cache('negarin.media_types'))->explode(','),
            'image_quality' => config_cache('negarin.image_quality'),
            'optimize_image' => (bool) config_cache('negarin.optimize_image'),
            'max_photo_size' => config_cache('negarin.max_photo_size'),
            'max_caption_length' => config_cache('negarin.max_caption_length'),
            'max_altext_length' => config_cache('negarin.max_altext_length'),
            'enforce_account_limit' => (bool) config_cache('negarin.enforce_account_limit'),
            'max_account_size' => config_cache('negarin.max_account_size'),
            'max_album_length' => config_cache('negarin.max_album_length'),
            'account_deletion' => (bool) config_cache('negarin.account_deletion'),
        ];

        if (config_cache('negarin.directory.testimonials')) {
            $testimonials = collect(json_decode(config_cache('negarin.directory.testimonials'), true))
                ->map(function ($t) {
                    return [
        ...
        $res['has_submitted'] = config_cache('negarin.directory.has_submitted') ?? false;
        $res['synced'] = config_cache('negarin.directory.is_synced') ?? false;
        $res['latest_response'] = config_cache('negarin.directory.latest_response') ?? null;
        ...
        $submissionState = Http::withoutVerifying()
            ->post('https://negarin.org/api/v1/directory/check-submission', [
    ...
    public function directoryStore(Request $request)
    {
        ...
        ConfigCacheService::put('negarin.directory', $config->v);
    ...
    public function directoryHandleServerSubmission(Request $request)
    {
        ...
        $reqs['feature_config'] = [
            ...
            'oauth_enabled' => (bool) config_cache('negarin.oauth_enabled'),
            'media_types' => Str::of(config_cache('negarin.media_types'))->explode(','),
            'image_quality' => config_cache('negarin.image_quality'),
            'optimize_image' => config_cache('negarin.optimize_image'),
            'max_photo_size' => config_cache('negarin.max_photo_size'),
            'max_caption_length' => config_cache('negarin.max_caption_length'),
            'max_altext_length' => config_cache('negarin.max_altext_length'),
            'enforce_account_limit' => config_cache('negarin.enforce_account_limit'),
            'max_account_size' => config_cache('negarin.max_account_size'),
            'max_album_length' => config_cache('negarin.max_album_length'),
            'account_deletion' => config_cache('negarin.account_deletion'),
        ];
        ...
        ConfigCacheService::put('negarin.directory.submission-key', Str::random(random_int(40, 69)));
        ConfigCacheService::put('negarin.directory.submission-ts', now());

        $data = (new negarinDirectoryController())->buildListing();
        $res = Http::withoutVerifying()->post('https://negarin.org/api/v1/directory/submission', $data);
    ...
    public function directoryDeleteBannerImage(Request $request)
    {
        ...
        $directory = ConfigCache::whereK('negarin.directory')->first();
        ...
        $bannerImage->save();
        Cache::forget('api:v1:instance-data-response-v1');
        ConfigCacheService::put('negarin.directory', $directory);
    ...
    public function directoryDeleteTestimonial(Request $request)
    {
        ...
        $testimonials = ConfigCache::whereK('negarin.directory.testimonials')->firstOrFail();
        $existing = collect(json_decode($testimonials->v, true))
            ->filter(function ($t) use ($profile_id) {
                return $t['profile_id'] !== $profile_id;
            })
            ->values();
        ConfigCacheService::put('negarin.directory.testimonials', $existing);
    ...
    public function directorySaveTestimonial(Request $request)
    {
        ...
        $configCache = ConfigCache::firstOrCreate([
            'k' => 'negarin.directory.testimonials',
        ]);
        ...
        $configCache->save();
        ConfigCacheService::put('negarin.directory.testimonials', $configCache->v);
        $res = [
