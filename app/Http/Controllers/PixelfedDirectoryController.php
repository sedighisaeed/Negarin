...
class negarinDirectoryController extends Controller
{
    public function get(Request $request)
    {
        ...
        if (! config_cache('negarin.directory.submission-key')) {
            abort(404);
        }

        if (! hash_equals(config_cache('negarin.directory.submission-key'), $request->input('sk'))) {
            abort(403);
        }
    ...
    {
        $res = config_cache('negarin.directory');
        if ($res) {
            $res = is_string($res) ? json_decode($res, true) : $res;
        }

        $res['_domain'] = config_cache('negarin.domain.app');
        $res['_sk'] = config_cache('negarin.directory.submission-key');
        $res['_ts'] = config_cache('negarin.directory.submission-ts');
        $res['version'] = config_cache('negarin.version');
        ...
        $openRegistration = (bool) config_cache('negarin.open_registration');
        ...
        $oauthEnabled = ConfigCache::whereK('negarin.oauth_enabled')->first();
        if ($oauthEnabled) {
        ...
        if (config_cache('negarin.directory.testimonials')) {
            $res['testimonials'] = collect(json_decode(config_cache('negarin.directory.testimonials'), true))
