...
class ComposeController extends Controller
{
    ...
    {
        $this->validate($request, [
            'caption' => 'nullable|string|max:'.config_cache('negarin.max_caption_length', 500),
            'media.*' => 'required',
            'media.*.id' => 'required|integer|min:1',
            'media.*.filter_class' => 'nullable|alpha_dash|max:30',
            'media.*.license' => 'nullable|string|max:140',
            'media.*.alt' => 'nullable|string|max:'.config_cache('negarin.max_altext_length'),
            'cw' => 'nullable|boolean',
        ...
        foreach ($medias as $k => $media) {
            if ($k + 1 > config_cache('negarin.max_album_length')) {
                continue;
            }
    ...
    public function mediaProcessingCheck(Request $request)
    {
        ...
        if (config('negarin.media_fast_process')) {
            return [
                'finished' => true,
            ];
        }
        ...
        switch ($media->mime) {
            ...
            case 'image/png':
            case 'video/mp4':
                $finished = (bool) config_cache('negarin.cloud_storage') ? (bool) $media->cdn_url : (bool) $media->processed_at;
                break;
    ...
    public function composeSettings(Request $request)
    {
        ...
        $types = config_cache('negarin.media_types');
        ...
        $default = [
            ...
            'max_file_size' => (int) config_cache('negarin.max_photo_size'),
            'max_media_attachments' => (int) config_cache('negarin.max_album_length'),
            'max_altext_length' => (int) config_cache('negarin.max_altext_length'),
        ];
